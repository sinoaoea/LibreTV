// 全局变量
let selectedAPIs = JSON.parse(localStorage.getItem('selectedAPIs') || '["tyyszy","mdzy", "bfzy", "ruyi","zy360","wolong","jisu","dbzy","zuid","wujin","ikun"]'); // 默认选中资源
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!localStorage.getItem('hasInitializedDefaults')) {
        // 默认选中资源
        selectedAPIs = ["tyyszy","mdzy", "bfzy", "ruyi","zy360","wolong","jisu","dbzy","zuid","wujin","ikun"];
...
function resetSearchArea() {
    ...
    try {
        window.history.pushState(
            {}, 
            `MiyingTV - 影视流媒体资源搜索平台 - 自由、纯净、无广告`, 
            `/`
        );
        // 更新页面标题
        document.title = `MiyingTV - 影视流媒体资源搜索平台 - 自由、纯净、无广告`;
    } catch (e) {
...
async function search() {
    ...
    try {
        ...
        try {
            ...
            // 更新页面标题
            document.title = `搜索: ${query} - MiyingTV`;
        } catch (e) {
            console.error('更新浏览器历史失败:', e);
            // 如果更新URL失败，继续执行搜索
        }
...
async function exportConfig() {
    ...
    config['name'] = 'MiyingTV-Settings';  // 配置文件名，用于校验
    ...
    // 将配置数据保存为 JSON 文件
    saveStringAsFile(JSON.stringify(config), 'MiyingTV-Settings_' + times + '.json');
}
